...
const config = {
  libraryName: '@angular-material-extensions/cards',
  unscopedLibraryName: 'cards',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /cards/ --deploy-url /cards/`, {cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, {cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run @angular-material-extensions/cards-demo:server`, {cwd: `${config.demoDir}`}))
